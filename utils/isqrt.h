...
namespace utils
{
	namespace internals
	{
		inline uint32_t isqrt_implementation(uint32_t guess_base, uint32_t input)
		{
			assert(input > 0);
			uint64_t guess = 1;
			while (true)
	...
	inline uint32_t isqrt(uint32_t input)
	{
		switch (input)
		{
		default:
			return internals::isqrt_implementation(0, input);
		case 0:
			return 0;
	...
	inline int32_t isqrt(int32_t input)
	{
		assert(input >= 0);
